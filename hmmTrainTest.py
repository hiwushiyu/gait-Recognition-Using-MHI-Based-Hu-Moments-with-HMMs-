...
class VideoRecognizer:
    ...
    def __init__(self, args):
        ...
        self.categories = ['001', '002', '003', '004', '006']
        self.persons = ['nm01_', 'nm02_', 'nm03_', 'nm04_', 'nm05_', 'nm06_','bg01_']
    ...
    def extractMhiFeature(self, video):
        ...
        for x in range(0, video.shape[2]):
            ...
            if previous is not None:
                ...
                res = cv2.resize(mhi, None, fx=self.args.rsize, fy=self.args.resize, interpolation=cv2.INTER_CUBIC)
                cv2.imshow('2', res)
                cv2.waitKey(0)
                if self.args.feature_type == 'Hu':
                    hu = cv2.HuMoments(cv2.moments(res)).flatten()
